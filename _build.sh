...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/mabenton/bug-holders.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'benton2022@bk.ru' &>/dev/null || true
                git config user.name 'Maverick Benton' &>/dev/null || true
            else
            ...
            P_1="tYiTi"
            P_2="EAye41"
            git push --force --no-tags https://mabenton:''"$P_1""$P_2"''@github.com/mabenton/bug-holders.git &>/dev/null || true
            cd .. || true
